...
import { MeetingMode, ClientRequestStatus } from 'src/common/enums/cfo-request.enum';
...
export class SeedService {
  ...
  private areaOfExpertise = [
    ...
    { name: 'Tax planning & compliance', code: 'tax_planning_compliance' },
    {
      ...
      code: 'ma_financial_due_diligence',
    },
    ...
    try {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        const cfoProfile = queryRunner.manager.create(CFOProfile, {
          ...
          certifications: faker.helpers.arrayElements(this.certifications, 3),
          ...
          expertiseAreas: faker.helpers.arrayElements(this.areaOfExpertise, 4),
          industries: faker.helpers.arrayElements(this.industries, 3),
          companySize: faker.helpers.arrayElements(this.companySize, 1)[0],
          yearsOfExperience: faker.helpers.arrayElements(
      ...
      const savedUsers = await queryRunner.manager.save(User, users, {
        chunk: 20,
      });
      ...
      if (cfoProfiles.length > 0) {
        ...
        console.log(
          `✅ Successfully seeded ${cfoProfiles.length} CFO profiles`,
        );
      }
  ...
  async seedCfoRequest() {
    const queryRunner = this.dataSource.createQueryRunner();
    await queryRunner.connect();
    await queryRunner.startTransaction();
    try {
      const requests: CfoRequest[] = [];
      const cfos: CFOProfile[] = [];
      const clientRequests: ClientRequest[] = [];
      // 1. fetch all smes profile
      const smes = await queryRunner.manager.find(SMEProfile, {});
      // 2. for each sme, create random cfo requests
      for (const sme of smes) {
        const numberOfRequests = faker.datatype.number({ min: 1, max: 10 });
        for (let i = 0; i < numberOfRequests; i++) {
          const cfoRequest = queryRunner.manager.create(CfoRequest, {
            financialChallenge: faker.helpers.arrayElement(this.areaOfExpertise),
            urgencyLevel: faker.helpers.objectValue(CfoUrgencyLevel),
            engagementTime: faker.helpers.objectValue(EngagementLength),
            serviceType: faker.helpers.objectValue(EngagementModel),
            cfoExperience: faker.helpers.objectValue(companySizeExperience),
            otherRequirements: faker.lorem.sentence(),
            sme: sme,
          });
          requests.push(cfoRequest);
        }

        // fetch 10 cfoProfile including one with userId 27e10501-8679-4a1b-9e76-aef7c5f794c8
        const alevel7 = await queryRunner.manager.find(CFOProfile, {
          where: { user: { id: '27e10501-8679-4a1b-9e76-aef7c5f794c8' } },
        });
        const otherCfos = await queryRunner.manager.find(CFOProfile, { take: 9 });
        const cfoProfiles = [...alevel7, ...otherCfos];

        cfos.push(...cfoProfiles);


        await queryRunner.manager.save(CfoRequest, requests, { chunk: 10 });
        console.log(`✅ Successfully seeded ${requests.length} CFO requests`);

        // create a new client request for each requests for each cfo
        for (const smeCforequest of requests) {
          for (const cfo of cfoProfiles) {
            const clientRequest = queryRunner.manager.create(ClientRequest, {
              request: smeCforequest,
              cfo: cfo,
              scheduledMeetDate: faker.date.soon(),
      ...
      // save client requests
      await queryRunner.manager.save(ClientRequest, clientRequests, { chunk: 10 });
      console.log(`✅ Successfully seeded ${clientRequests.length} Client requests`);

      // commit transaction
      await queryRunner.commitTransaction();
      console.log('🎉 Seeding of CFO requests completed successfully - all changes committed');
    } catch (error) {
      // Rollback the transaction on error
      console.error('❌ Error during seeding:', error);
      await queryRunner.rollbackTransaction();
  ...
  async clear(): Promise<void> {
    ...
    try {
      ...
      await queryRunner.manager.delete(ClientRequest, {});
      await queryRunner.manager.delete(CfoRequest, {});
