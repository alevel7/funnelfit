...
import { engagementSimilarity, experienceSimilarity, serviceTypeSimilarity, urgencySimilarity } from './helper/partial-match.config';
...
export class SmeService {
    constructor(
        ...
        @InjectRepository(CfoRequest) private readonly cfoRequestRepo: Repository<CfoRequest>,
    ...
    async getCFORequestResult(requestId: string, page: number) {
        console.log("Fetching CFO request result for requestId:", requestId, "page:", page);
    ...
    async sendRequestToCfo(requestId: string, body: ClientRequestDto) {
        ...
        const cachedCfos = await this.cacheManager.get<CFOProfile[]>(`cfo-requests-${requestId}`) ?? [];
        ...
        return SendResponse.success(clientRequest, 'Request sent to CFO successfully');
    }
    ...
    async requestCFO(data: CfoRequestDto, user: LoggedInUser, page = 1, limit = 3) {
        ...
        const smeProfile = await this.smeRepo.findOne({ where: { user: { id: user.id } } });
        if (!smeProfile) {
            throw new NotFoundException('SME Profile not found');
        }
    ...
    private computeMatchScore(cfo: CFOProfile, dto: CfoRequestDto): number {
        ...
        // --- Financial Challenge ---
        if (cfo.expertiseAreas?.map(p => p.code)?.includes(dto.financialChallenge.code))
